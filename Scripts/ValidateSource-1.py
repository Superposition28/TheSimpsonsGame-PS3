...
def check_required_directories(base_folder: str, required_dirs: list) -> bool:
    ...
    """
    print(prefix="Validate", message=f"--- Checking for Required Subdirectories in: {base_folder} ---")
    all_found = True
    missing_dirs = []
    found_dirs_count = 0
    ...
    if not all_found:
        ...
        print(colour=Colours.GREEN, prefix="Validate", message=f"✅ All {len(required_dirs)} required subdirectories found.")
    print(prefix="Validate", message=f"-" * 20)
    return all_found
...
def check_file_existence(db_path: str, base_check_folder: str):
    ...
    if not os.path.isfile(db_path):
        print(prefix="Validate", message=f"ERROR: Database file not found at {db_path}")
        return

    if not os.path.isdir(base_check_folder):
        print(prefix="Validate", message=f"ERROR: Check folder not found or is not a directory: {base_check_folder}")
        return
    ...
    try:
        ...
        print(prefix="Validate", message=f"\n--- Starting Database File Existence Check ---")
        print(prefix="Validate", message=f"Database: {db_path}")
        print(prefix="Validate", message=f"Checking against base folder: {base_check_folder}\n")

        for table_name, path_column_name in tables_to_check.items():
            print(prefix="Validate", message=f"Checking table: {table_name}...")
            try:
                cursor.execute(f"SELECT {path_column_name} FROM {table_name}")
                rows = cursor.fetchall()
            except sqlite3.OperationalError as e:
                print(prefix="Validate", message=f"  WARNING: Could not query table {table_name}. Error: {e}")
                print(prefix="Validate", message=f"  Skipping this table.")
                print(prefix="Validate", message=f"-" * 20)
                continue
            ...
            for row_index, row in enumerate(rows):
                ...
                if relative_path is None:
                    print(prefix="Validate", message=f"  WARNING: Row {row_index + 1} in {table_name} has a NULL {path_column_name}. Skipping.")
                    continue
                ...
                if not os.path.isfile(full_expected_path):
                    #print(prefix="Validate", message=f"  MISSING (DB): {full_expected_path} (from {table_name})")
                    overall_db_files_all_found = False
                    total_db_files_missing += 1
                    table_missing_count +=1
                # else:
                #     print(prefix="Validate", message=f"  FOUND (DB): {full_expected_path}") # Optional: for verbose output
            ...
            if table_checked_count > 0: # Only print summary if files were expected
                if table_missing_count == 0:
                    print(prefix="Validate", message=f"  All {table_checked_count} files from {table_name} (as per DB) found.")
                else:
                    print(prefix="Validate", message=f"  {table_missing_count} file(s) MISSING from {table_name} (out of {table_checked_count} expected in this table).")
            else:
                print(prefix="Validate", message=f"  No valid file paths to check in {table_name}.")
            print(prefix="Validate", message=f"-" * 20)

        print(prefix="Validate", message=f"\n--- Overall Summary ---")
        if not required_dirs_all_present:
            print(prefix="Validate", message=f"NOTE: Some required base subdirectories were missing. This might affect the accuracy of the database file checks.")
        
        if total_db_files_checked == 0:
            print(prefix="Validate", message=f"No files were checked from the database. Ensure tables exist, contain data, and paths are valid.")
        elif overall_db_files_all_found:
            print(prefix="Validate", message=f"✅ All {total_db_files_checked} files checked from the database were found in {base_check_folder}.")
        else:
            print(prefix="Validate", message=f"❌ {total_db_files_missing} out of {total_db_files_checked} files checked from the database are MISSING.")

    except sqlite3.Error as e:
        print(prefix="Validate", message=f"SQLite error: {e}")
    except Exception as e:
        print(prefix="Validate", message=f"An unexpected error occurred: {e}")
    finally:
        if conn:
            conn.close()
