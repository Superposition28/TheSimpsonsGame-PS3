...
def rle_tokens(tokens):
    """Run-length encode list of 4-byte tokens (each token is 8 hex chars or '????????').
    Returns list of dicts: {'t': token, 'n': count}
    """
    out = []
    if not tokens:
        return out
    ...
    for t in tokens[1:]:
        if t == cur:
            ...
            out.append({'t': cur, 'n': n})
            cur = t
            n = 1
    out.append({'t': cur, 'n': n})
    return out

def rle_preview(rle, max_groups=24):
    pieces = []
    for g in rle[:max_groups]:
        if g['n'] == 1:
            pieces.append(g['t'])
        else:
            pieces.append(f"{g['t']}×{g['n']}")
    if len(rle) > max_groups:
        pieces.append("...")
    return " ".join(pieces)

def token_entropy_32b(tok: bytes) -> float:
    """Shannon entropy over 4 bytes, in bits (0..32)."""
    counts = Counter(tok)
    n = 4
    h = 0.0
...
def mostly_zero(tok: bytes, thresh=3):
    return sum(1 for b in tok if b == 0) >= thresh
...
def find_anchors(files, max_bytes, topk, progress=True,
                                 anchor_min_entropy=8.0,
                                 anchor_max_global_pct=0.25,
                                 anchor_stoplist=None,
                                 avoid_zero_anchors=False):
    """
    Returns:
    ...
    if anchor_stoplist:
        try:
            stop = {bytes.fromhex(s.strip().lower()) for s in anchor_stoplist.split(",") if s.strip()}
        except ValueError:
            stop = set()
    ...
    for tok, fset in token_files.items():
        if tok in stop:
            continue
        if avoid_zero_anchors and mostly_zero(tok):
            continue
        if token_entropy_32b(tok) < anchor_min_entropy:
            continue
        if (len(fset) / max(1, num_files)) > anchor_max_global_pct:
            # too ubiquitous; likely padding/format boilerplate
            continue
        candidates.append((tok, fset))

    # Rank by file support (desc) then by entropy (desc) to break ties
    candidates.sort(key=lambda kv: (-len(kv[1]), -token_entropy_32b(kv[0])))
    global_rank = candidates[:topk]
...
def main():
    ...
    ap.add_argument("--anchor-min-entropy", type=float, default=8.0,
                    help="Minimum Shannon entropy (bits) for a 4-byte anchor (0..32). Default 8.0")
    ap.add_argument("--anchor-max-global-pct", type=float, default=0.25,
                    help="Max fraction of files containing an anchor to consider it (0..1). Default 0.25")
    ap.add_argument("--anchor-stoplist", type=str, default="00000000,0000803f,3f800000,3f000000,00000080,00803f00",
                    help="Comma-separated hex 8 list of anchors to ignore (case-insensitive)")
    ap.add_argument("--avoid-zero-anchors", action="store_true",
                    help="Exclude anchors with >=3 zero bytes (default off)")
    ...
    if not args.no_consensus:
        ...
        for r in res1:
            ...
            # RLE preview
            rle = rle_tokens(r["pattern_tokens"]) if r.get("pattern_tokens") else []
            print(" " + rle_preview(rle))
            # also attach RLE in result
            r["pattern_rle"] = rle
    ...
    if not args.no_anchor:
        ...
        global_rank, per_file_rank = find_anchors(
            files, args.max_bytes, args.anchor_topk, progress=(not args.no_progress),
            anchor_min_entropy=args.anchor_min_entropy,
            anchor_max_global_pct=args.anchor_max_global_pct,
            anchor_stoplist=args.anchor_stoplist,
            avoid_zero_anchors=args.avoid_zero_anchors
        )
        ...
        if not single_file_mode:
            ...
            if pat:
              ...
              print(" " + rle_preview(pat.get("pattern_rle", rle_tokens(pat.get("pattern_tokens", [])))))
        ...
        for fp, counter in per_file_rank.items():
            ...
            if file_patterns:
                ...
                for pat in file_patterns[:3]:  # show a few
                    print(f"  - anchor {pat['anchor']} : " + rle_preview(pat.get("pattern_rle", rle_tokens(pat.get("pattern_tokens", [])))))
