...
OOT = r"A:\Dev\github\yggdrasil-au\public\RemakeEngine\RemakeEngine\RemakeRegistry\Games\TheSimpsonsGame-PS3\reversing\Source\Format-Analysis\preinstanced\ps3\dff"
ROOT = r"A:\Dev\github\yggdrasil-au\public\RemakeEngine\RemakeEngine\RemakeRegistry\Games\TheSimpsonsGame-PS3\reversing\Source\Format-Analysis\preinstanced\ps3\rws"

DRY_RUN: bool = True  # set to False to actually move/rename
...
def main() -> None:
    ...
    for dp, _, files in os.walk(root):
        for fn in files:
            fl = fn.lower()
            if ".preinstanced" not in fl:
                continue
            if ".rws." not in fl and ".dff." not in fl:
                continue
            ...
            if DRY_RUN:
                ...
                if dest.exists():
                    ...
                    dest = dest.with_name(base + "_dup" + e)
                full.rename(dest)
    ...
    map_json = Path(ROOT) / "flatten_map.json"
    write_json(map_json, mapping_rows)

    created: List[str] = []
    for base, rows in per_base.items():
        safe = sanitize_filename(base)
        per_path = Path(ROOT) / f"flatten_map_{safe}.json"
    ...
    summary_json = Path(ROOT) / "flatten_map_summary.json"
