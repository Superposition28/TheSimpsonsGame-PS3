...
# --------------------------------------------------
# Coordinate mapping: Simpsons -> Blender
# X stays X
# Blender Z comes from game Y (Y-up -> Z-up)
# Blender Y comes from game Z, but flipped to opposite side
# --------------------------------------------------
...
def parse_tsg_graph(filepath):
    ...
    Returns a dict:
    {
        ...
        "vertices": [ (x,y,z), ... ],      # coord-ref block (experimental)
        "polygons": [ [idx0, idx1, ...], ... ]   # indices into vertices (experimental)
    }
    """
    ...
    # ------------ Parse coord-ref vertices (off48) (experimental) ------------
    vertices = []
    if 0 < off48 < size:
    ...
    # ------------ Parse index lists (off44) into polygons (experimental) ------------
    polygons = []
...
def make_polygons_mesh(context, graph_data, base_name):
    ...
    # --- hide polygons by default (viewport + render) ---
    obj.hide_set(True)
