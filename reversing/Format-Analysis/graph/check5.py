...
- Treat 0x10..0x1F as a 16-byte GUID, not a magic constant.
- Explicitly recognize [0x80 .. node_offset) as a 'header-ext' block
  containing a single float parameter at +8.
- Treat 0x60 as an additional offset (off60) and 0x64 as a small count
  (count_64), not another offset.
- Classify the block at off60 as 'offset-table' when it looks like a table of
...
class Header:
    ...
    guid_bytes: bytes        # 16-byte GUID at 0x10..0x1F
    ...
    off48: int
    off60: int
    count_64: int            # NOT an offset; small count
    off68: int
    off70: int
    val74: int
...
def parse_header(data: bytes) -> Header:
    ...
    guid_bytes = data[0x10:0x20]         # full 16-byte GUID
...
def parse_edges(data: bytes, hdr: Header) -> List[Edge]:
    ...
    # Candidate end for edges: first header offset after node_end
    candidate_offsets = [
        x for x in (hdr.off40, hdr.off44, hdr.off48, hdr.off60, hdr.off68, hdr.off70)
        if x != 0 and x > start
    ]
...
def discover_blocks(data: bytes, hdr: Header) -> List[BlockInfo]:
    ...
    offsets.add(0x00)              # file start
    offsets.add(0x80)              # end of main header
    offsets.add(hdr.node_offset)
    offsets.add(hdr.node_end)

    for x in (hdr.off40, hdr.off44, hdr.off48, hdr.off60, hdr.off68, hdr.off70):
        if 0 < x < size:
            offsets.add(x)
    ...
    def classify_block(start: int, end: int) -> Tuple[str, str]:
        ...
        # Header extension: [0x80 .. node_offset)
        if start == 0x80 and end == hdr.node_offset and length >= 0x14:
            # This block consistently looks like:
            #   0x00..0x07:  0
            #   0x08..0x0B:  float parameter
            #   0x0C..0x13:  0
            fparam = f32be(data, start + 8)
            return "header-ext", f"single float param ~{fparam:.7f}"
        ...
        # Block at off60: extra offset table when count_64 > 0
        if start == hdr.off60 and hdr.count_64 > 0 and length >= 0x20:
            # Check for a pattern of alternating zero and increasing multiples of 0x10
            u32s = [u32be(data, start + i * 4) for i in range(min(length // 4, 16))]
            # Rough pattern: 0, 0x10, 0, 0x20, 0, 0x30, ...
            good = 0
            for i, v in enumerate(u32s):
                if i % 2 == 0 and v == 0:
                    good += 1
                elif i % 2 == 1 and (v % 0x10 == 0 and v > 0):
                    good += 1
            ratio = good / max(1, len(u32s))
            if ratio > 0.6:
                return "offset-table", f"{hdr.count_64} entries (alt zero & 0x10-multiples)"
...
HEADER_OFFSET_FIELDS = ("off40", "off44", "off48", "off60", "off68", "off70")
...
def guid_str(hdr: Header) -> str:
    b = hdr.guid_bytes
    # Simple hex; if you want proper GUID formatting, you can adapt:
    return b.hex()
...
def print_file_detail(fl: FileLayout, verbose: int) -> None:
    ...
    print(f"  off60/64/68:  0x{hdr.off60:08X} count_64={hdr.count_64} 0x{hdr.off68:08X}")
