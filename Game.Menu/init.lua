...
local AUDIO_LANG_FOLDERS = { EN=false, ES=false, FR=false, IT=false, Global=false }
...
local function create_godot_project(project_name, project_path, extracted_root, scripts_folder, addons_folder, conf_folder, godot_exe, no_exit, logo_images, asset_exts)
    ...
    -- Run scene builder
    local cmd = { godot_exe, "--editor", "--path", project_dir, "--script", "res://Scripts/import-Gem.gd" }
...
-- Discover PNG files only in the specified directory and single parent and child.
...
local function run()
    ...
    if not opts["repo-root"] or not opts["sourcePath"] then
        error("Missing required args: --repo-root and --sourcePath, args: " .. table.concat(args, " "))
    end
