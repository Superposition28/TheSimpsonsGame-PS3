...
--[[
todo, implement creation of the Music folder in the 'A1_Audio' folder, then run quickbms to extract the .mus into its sns and snr files,
then run vgmstream to convert them into wav for each mus output folder
then delete the sns and snr files, leaving only the wav files and the mus files
--]]
...
-- powershell example of how to run this script
   however the lua version will use the lua engines Tool calls for quickbms and vgmstream, so you don't need to install them or have them in your path
   additionally the folder paths will be relative and most will be args passed into the script
