...
def build_texture_mesh_links(data: bytes) -> dict[int, list[str]]:
    """
    ...
    Pass 2: walk events in order and associate the last texture set (after TLFD if present)
            to the next mesh encountered. Returns {mesh_chunk_offset: [tex_names]}.
    """
...
class SimpGameImport(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context: bpy.types.Context) -> set:
        ...
        texture_links_by_mesh_offset = build_texture_mesh_links(tmpRead)
            ...
            for i in range(mDataSubCount):
                ...
                try:
                    ...
                    for v in range(VertCount):
                        ...
                        # --- ###################################################### ---
                        # --- START: UV FIX (explicit reads + diagnostics) ---
                        # --- ###################################################### ---
                ...
                # --- START: TYPO FIX ---
                # --- ###################################################### ---
                # obj = bpy.data.objects.new(f"Mesh_{mesh_shorter}_{i}", mesh1) # This was the typo
                obj = bpy.data.objects.new(f"Mesh_{mesh_iter}_{i}", mesh1) # Corrected to mesh_iter
                # --- ###################################################### ---
                # --- END: TYPO FIX ---
                # --- ###################################################### ---
                ...
                try:
                    if uv_layer_name in mesh.uv_layers:
                        ...
                        # Optionally de-prioritize CM layer for export
