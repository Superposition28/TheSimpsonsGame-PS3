...
# NEW: Applies specified actions to a node after it has been instantiated.
func _apply_post_load_actions(node: Node, actions: Array):
	print("    Applying post-load actions for '", node.name, "'")
	for action_info in actions:
		if not action_info is Dictionary:
			printerr("      Skipping invalid action (not a dictionary): ", action_info)
			continue

		var action_type = action_info.get("action")
		if action_type == "set_property":
			var target_path = action_info.get("target_path")
			var property_name = action_info.get("property")
			var value = action_info.get("value")

			if not target_path or not property_name:
				printerr("      Skipping 'set_property' action: missing 'target_path' or 'property'.")
				continue
			
			# Find the target node within the newly instantiated scene/model.
			var target_node = node.get_node_or_null(NodePath(target_path))

			if target_node:
				target_node.set(property_name, value)
				print("      -> Set property '", property_name, "' to '", value, "' on target '", target_path, "'")
			else:
				printerr("      -> ERROR: Could not find target node at path '", target_path, "' within '", node.name, "'")
		else:
			printerr("      Skipping unknown action type: '", action_type, "'")


# REWRITTEN FUNCTION: Creates collision bodies in a separate container node.
func _generate_collision_for_node(node_to_scan: Node, collision_parent: Node3D, scene_root: Node):
	# This function traverses the visual node tree (`node_to_scan`).
	# For each mesh it finds, it creates a corresponding StaticBody3D in the `collision_parent`.

	# Check if the current node is a MeshInstance3D with a valid mesh.
	var mesh_instance = node_to_scan as MeshInstance3D
	if mesh_instance and mesh_instance.mesh:

		# Create the physics body and its shape.
		var static_body = StaticBody3D.new()
		var shape_node = CollisionShape3D.new()

		# Generate the collision data from the visual mesh.
		...
		# Add the shape to the body, and the body to our dedicated collision container.
		static_body.add_child(shape_node)
		collision_parent.add_child(static_body)

		# Set owner so they are saved with the level scene.
		static_body.owner = scene_root
		shape_node.owner = scene_root

		# CRITICAL: Copy the exact world transform from the visual mesh to the new static body.
		# This ensures the collision shape is perfectly aligned with the visual model.
		static_body.global_transform = mesh_instance.global_transform
		# Apply a 90-degree rotation on the X-axis to correct for the Z-up vs Y-up coordinate difference.
		static_body.rotate_x(deg_to_rad(90))
		print("      -> Generated collision body for '", mesh_instance.name, "'")

	# Recurse through all children of the visual node to find all nested meshes.
...
func _add_children_recursive(parent_node: Node, children_data: Array, scene_root: Node) -> bool:
	...
	for child_info in children_data:
		...
		# --- MODIFIED SECTION: COLLISION GENERATION ---
		if child_info.has("collision"):
			...
			if collision_config is Dictionary and collision_config.get("enabled", false):
				...
				# Create a sibling node to hold all the collision bodies for this instance.
				var collision_container = Node3D.new()
				collision_container.name = instance.name + "_Collision"
				parent_node.add_child(collision_container) # Added as a sibling to `instance`.
				collision_container.owner = scene_root

				# Call the generator, passing the visual instance to scan and the new container to populate.
				_generate_collision_for_node(instance, collision_container, scene_root)
		# --- END OF MODIFIED SECTION ---
		
		# --- NEW SECTION: POST-LOAD ACTIONS ---
		if child_info.has("post_load_actions"):
			_apply_post_load_actions(instance, child_info["post_load_actions"])
		# --- END OF NEW SECTION ---
...
# (The rest of the script: _apply_node_config, _init, etc. remains exactly the same)
# ...
...
func _init():
	...
	await create_timer(3).timeout # Reduced wait time

	# Check for the --no-exit command-line argument.
	if OS.get_cmdline_args().has("--no-exit"):
		print("\n'--no-exit' flag detected. Godot will remain open.")
	else:
		print("\nWaiting before exiting...")
		await await create_timer(3).timeout
		quit()
